...
const Intro = ({ tracks, onTrackSelect, selectedTrackId = null, isVisible = true, isDirectUri = false }) => {
  ...
  useEffect(() => {
    ...
    if (isVisible) {
      ...
      const mainCroquetaIndex = tracks.findIndex(track => 
  ...
  return (
    ...
    >
      <div className={`${MAINCLASS}__container`} ref={containerRef}>
        ...
        {selectedTrackId && tracks.map((track, index) => {
